...
- [Деплой на Raspberry Pi 4](#деплой-на-raspberry-pi-4)
- [Сравнение платформ деплоя](#сравнение-платформ-деплоя)
...
Домашний сервер 24/7: тот же стек (Docker Compose), удалённая PostgreSQL, **polling** — порты в роутере открывать не нужно.

### Схема

```
[Raspberry Pi 4]
   Docker → контейнер telegram-moderation-bot
        ├── api.telegram.org (исходящий)
        └── PostgreSQL на удалённом хосте (исходящий)
   ./logs/bot.log на microSD
```
...
| RAM | **2 GB** минимум, лучше **4 GB** |
| Карта microSD | 16 GB+, класс A2 |
| ОС | **Raspberry Pi OS (64-bit)** — Bookworm |
| Питание | Официальный 5V 3A (USB-C) |
| Сеть | Ethernet (предпочтительно) или Wi‑Fi |
| PostgreSQL | Удалённая БД (на Pi не поднимать) |

> **QNAP TS-219P** и другие старые NAS (512 MB RAM, без Docker) для этого проекта **не подходят**. Pi 4 — рабочая альтернатива «дома».

### 1. Установка Raspberry Pi OS

1. [Raspberry Pi Imager](https://www.raspberrypi.com/software/) → **Raspberry Pi OS (64-bit)**.
2. В настройках Imager (⚙):
   - включить **SSH**;
   - задать пользователя и пароль;
   - при необходимости — Wi‑Fi.
3. Записать образ, загрузить Pi, узнать IP (роутер или `ping raspberrypi.local`).
...
Выйдите из SSH и войдите снова. Проверка:
...
Заполните `TELEGRAM_BOT_TOKEN` и `DB_*` (как на ПК). Сохранение: `Ctrl+O`, Enter, `Ctrl+X`.

**Whitelist PostgreSQL:** на хостинге БД разрешите IP вашего домашнего интернета (или диапазон провайдера). После смены внешнего IP доступ может пропасть — обновите правила firewall.

Проверка связи с БД (опционально):
...
Первая сборка на ARM занимает **5–15 минут**.
...
Ожидаемые строки: `Connected to PostgreSQL`, `Bot started, polling updates`.

Логи на Pi:

```bash
tail -f ~/Moderator_bot/logs/bot.log
```

Не забудьте [настройку Telegram](#настройка-telegram): Privacy Mode **выключен**, бот — **админ** с правом удалять сообщения.

### 5. Автозапуск после перезагрузки Pi

В `docker-compose.yml` уже есть `restart: unless-stopped`. Дополнительно:
...
```bash
docker compose -f ~/Moderator_bot/docker-compose.yml ps
```
...
### 8. Запуск без Docker (экономия RAM на 2 GB)

Если Docker ощущается тяжёлым:
...
Автозапуск через **systemd** — файл `/etc/systemd/system/moderation-bot.service`:
...
Type=simple
User=pi
WorkingDirectory=/home/pi/Moderator_bot
EnvironmentFile=/home/pi/Moderator_bot/.env
ExecStart=/home/pi/Moderator_bot/.venv/bin/python -m bot.main
Restart=always
RestartSec=10
...
sudo systemctl enable moderation-bot
sudo systemctl start moderation-bot
journalctl -u moderation-bot -f
```

Замените `pi` и путь `/home/pi/Moderator_bot` на свои.

### 9. Рекомендации для Pi 4

| Тема | Совет |
|------|--------|
| RAM 2 GB | Не запускайте PostgreSQL на Pi; лишние контейнеры закройте |
| SD-карта | Логи ротируются (5×5 MB); периодически `docker system prune` |
| Перегрев | Корпус с охлаждением; при throttling CPU бот может тормозить |
| Статический IP | Закрепите IP Pi в роутере — проще whitelist в БД |
| Отключение света | UPS для Pi или допустимые простои Telegram |
| ARM64 | Образ `python:3.12-slim` на 64-bit OS собирается штатно |

Ориентир: контейнер бота ~**100–200 MB RAM**, CPU в простое почти нулевой.

### 10. Проблемы на Raspberry Pi

| Симптом | Решение |
|---------|---------|
| `Cannot connect to the Docker daemon` | `sudo systemctl start docker`; пользователь в группе `docker` |
| Сборка падает / зависает | `free -h` — мало RAM; увеличить swap (см. ниже) |
| Нет связи с PostgreSQL | Whitelist IP; `ping DB_HOST` |
| Бот не видит сообщения | BotFather → Group Privacy → **Turn off** |
| После обновления ОС | `docker compose up -d --build` |

**Увеличение swap** (если мало RAM при сборке):
...
### 11. Pi как хранилище логов (опционально)

Бот может работать на VPS/ПК, а каталог `logs/` монтировать с Pi по **NFS/SMB** — архив и бэкап. Для простого сценария достаточно локальной папки `logs/` на том же Pi, где крутится контейнер.

---

## Сравнение платформ деплоя

| Платформа | Docker Compose | 24/7 | Сложность | Примечание |
|-----------|:--------------:|:----:|:---------:|------------|
| **Windows + Docker Desktop** | ✅ | если ПК включён | Низкая | `start.bat` / `stop.bat` |
| **Raspberry Pi 4** | ✅ | ✅ дома | Средняя | 2–4 GB RAM, 64-bit OS |
| **VPS (Ubuntu)** | ✅ | ✅ | Средняя | Стабильнее домашнего IP |
| **QNAP TS-219P** | ❌ | — | — | 512 MB RAM, без актуального Docker |
| **GitHub Actions + VPS** | ✅ | ✅ | Выше | Автодеплой по push |
